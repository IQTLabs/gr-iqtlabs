...
bin/check_blocks.py && \
  ...
  ./bin/clonedeps.sh && \
  rm -rf build && mkdir build && cd build && cmake .. && make -j $(nproc) && sudo make install && sudo ldconfig && TEST_SIGMF=$(which sigmf_validate) make test ARGS="--verbose --timeout 180" && cd .. && \
